...
Before every tool call the agent consults a policy — mirroring the allow/deny
decision Goose's permission layer makes, but configured up front since Boitata
runs unattended.
...
are confined to a workspace root — by default the directory Boitata runs in.
...
- `McpServerConfig` redacts `auth_token` and the values of `env`/`headers`, which
